...
if ($action === 'audit' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $items = $metHistory->findGlobal([
        'tipo_evento' => 'bitacora_equipo_editada',
        'bitacora_equipo_id' => $auditId,
    ]);
    ...
    foreach ($items as $i => $it) {
        ...
        $label = trim((string)($meta['actor_nombre'] ?? ''));
        if ($label === '') {
            $uid = (string)($it['actor_user_id'] ?? '');
            if ($uid !== '') {
                $u = $userRepo->find($uid);
                if (is_array($u)) {
                    $label = trim((string)($u['nombre'] ?? ''));
                }
            }
        }
        $items[$i]['actor_label'] = $label !== '' ? $label : (string)($it['actor_user_id'] ?? '');
    }
...
if ($cambios !== []) {
    ...
    $metHistory->log(null, $actorId, 'bitacora_equipo_editada', 'Registro de bitácora modificado.', [
        'bitacora_equipo_id' => $id,
        'no_serie' => (string)($saved['no_serie'] ?? ''),
        'folio' => (string)($saved['folio'] ?? ''),
        'cambios' => $cambios,
        'actor_rpe' => strtoupper(trim((string)($user['rpe'] ?? ''))),
        'actor_nombre' => trim((string)($user['nombre'] ?? '')),
    ]);
}
