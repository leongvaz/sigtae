...
// En este módulo estamos en /metrologia/sig. Para enlaces globales (login/logout/dashboard),
// necesitamos la raíz de la app.
$appBasePath = preg_replace('#/metrologia/sig$#', '', $basePath);

$auth = sigtae_auth_service($container, $appBasePath);
$user = $auth->requireAuth();

$nombre = $user['nombre'] ?? ($user['rpe'] ?? 'Usuario');
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body class="bg-light min-vh-100">
    <header class="bg-white border-bottom shadow-sm">
        <div class="container-fluid px-4 py-3 d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <a href="<?= htmlspecialchars($appBasePath) ?>/logout.php" class="btn btn-outline-secondary btn-sm">Cerrar sesión</a>
            </div>
        </div>
    </header>
    ...
    <script>
        window.__SIGTAE_APP_BASE = <?= json_encode($appBasePath) ?>;
    </script>
