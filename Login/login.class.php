...
class LoginUser
{
	...
	{
		// Verificar si hay una URL de retorno guardada (ej: index.php, captura.php, comparativo.php?rpu=xxx)
		if (isset($_SESSION['redirect_after_login']) && !empty($_SESSION['redirect_after_login'])) {
			$redirect = $_SESSION['redirect_after_login'];
			unset($_SESSION['redirect_after_login']); // Limpiar para que no se use de nuevo
			// Si viene path absoluto (ej: /medicion/2026/comparativo.php), quedarnos solo con archivo?query
			if (strpos($redirect, '/') === 0) {
				$q = strpos($redirect, '?');
				$path = ($q !== false) ? substr($redirect, 0, $q) : $redirect;
				$query = ($q !== false) ? substr($redirect, $q) : '';
				$redirect = basename($path) . $query;
			}
