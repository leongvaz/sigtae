(function () {
    ...
    var base = location.pathname.replace(/\/index\.php$/, '').replace(/\/$/, '') || '';
    ...
    function loadSections() {
        ...
        fetch(api('sections.php'), fetchOptions())
            .then(function (r) {
                if (r.status === 401) {
                    var u = new URL(r.url);
                    location.href = base + '/Login/login.php' + (r.status === 401 ? '?expired=1' : '');
    ...
    function loadNodesInModal() {
        ...
        fetch(api('nodes.php') + q, fetchOptions())
            .then(function (r) {
                if (r.status === 401) { location.href = base + '/Login/login.php?expired=1'; return Promise.reject(); }
                return r.json();
            })
            .then(function (nodes) {
    ...
    function deleteDocument(docId) {
        fetch(api('document_delete.php'), fetchOptions({
            ...
            .then(function (r) {
                if (r.status === 401) { location.href = base + '/Login/login.php?expired=1'; return; }
                return r.json();
            })
            .then(function (data) {
                if (data && data.ok) {
